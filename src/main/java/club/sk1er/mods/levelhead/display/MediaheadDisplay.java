...
import club.sk1er.mods.levelhead.renderer.MediaheadTag;
...
public class MediaheadDisplay extends LevelheadDisplay {


    public MediaheadDisplay(MediaheadConfig config) {
        super(DisplayPosition.MEDIAHEAD, config);
    }
    ...
    public LevelheadTag buildTag(Levelhead levelhead, JsonHolder object, UUID uuid, LevelheadDisplay display, boolean allowOverride) {
        MediaheadTag mediaheadTag = new MediaheadTag(uuid);
        mediaheadTag.apply(levelhead, object);
        return mediaheadTag;
    }
}
