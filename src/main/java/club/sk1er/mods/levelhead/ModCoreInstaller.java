...
public class ModCoreInstaller {
    ...
    public static int initialize(File gameDir, String minecraftVersion) {
        ...
        dataDir = new File(gameDir, "sk1er_core");
        if (!dataDir.exists()) {
            if (!dataDir.mkdirs()) {
        ...
        if (!modcoreFile.exists() || !localMetadata.optString(minecraftVersion).equalsIgnoreCase(latestRemote)) {
            ...
            if (!download("https://static.sk1er.club/repo/mods/sk1er_modcore/" + latestRemote + "/" + minecraftVersion + "/ModCore-" + latestRemote + " (" + minecraftVersion + ").jar", latestRemote, modcoreFile, minecraftVersion, localMetadata)) {
